...
const ProjectController = {
  ...
  createProject: async (req, res) => {
    try {
      ...
      await ProjectMemberModel.create({
        ...
        userId,
        roleId: "6833dcef2fc333a1feca8ec5", // Hoặc roleId của leader nếu có
      });
      ...
      const reqNotification = {
        authorId: userId,
        projectId: project._id,
        content: `created a new project: ${projectName}`,
        Types: Enum.NOTIFICATION_TYPES.PROJECT_CREATED, // Hoặc loại thông báo
      };
  ...
  getAllProjects: async (req, res) => {
    try {
      ...
      const {
        status,
        from, // ISO string hoặc timestamp
        to, // ISO string hoặc timestamp
        sort = "desc", // asc hoặc desc
        page = 1,
        limit = 10,
      } = req.query;

      // Tìm tất cả projectId mà user đang là member
      const membershipList = await ProjectMemberModel.find({ userId }).select(
        "projectId",
      );

      const projectIds = membershipList.map((m) => m.projectId);
